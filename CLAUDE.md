...
Phase 2  핵심 차별점 완성 (Week 17~28)
  [x] 2-1 분석 고도화 (기초분석, PROCESS, PLS-SEM, MGA, CMV, 분석옵션)
  [x] 2-2 설문 모듈 (표본크기, 빌더, 그룹링크 배포, 응답자화면, 대시보드, DB뷰)
  [x] 2-3 조직·협업 (조직CRUD, 관리자대시보드, 랩워크스페이스, 검토요청, 코멘트, 승인, 알림)

Phase 3  E2E 완성 (Week 29~40)
  [x] 3-1 HLM 다층분석 (R엔진 HLM엔드포인트, ICC·rwg사전검증UI, HLM분석4탭UI, 한국어해석)
  [x] 3-2 E2E 플로우 완성 (프로젝트홈대시보드, 설문→캔버스자동연결, 보고서생성, 데이터품질게이트, 분석히스토리)
  [x] 3-3 배포·최종점검 (환경변수검증, R엔진재시도, 에러·404페이지, 로딩스켈레톤, README, DEPLOYMENT, next.config점검)
```
